...
def run_dynamo_export(
    ...
    batch_size, sequence_length, max_sequence_length = 2, 8, 10
...
def convert_to_float16(args: argparse.Namespace, fp32_path: str, rank: int = 0):
...
def get_args():
    ...
    parser.add_argument(
        ...
        choices=[Precision.FLOAT32, Precision.FLOAT16, Precision.INT4],
...
def main():
        ...
        if i == rank:
            ...
            if not os.path.exists(fp32_path):
                if args.use_dynamo_export:
                    ...
                    run_torchscript_export(args, l_config, llama, rank, world_size)
            del llama  # Delete LLaMA model from memory since it will be loaded again during parity check

            # Set model path to store FP32 optimized model
            fp32_opt_path = os.path.join(
                args.output, f"rank_{rank}_{args.model_name}_decoder_merged_model_fp32_opt.onnx"
            )

            # Run the optimizer script
            logger.info("Optimizing models...")
            optimize_export(args, l_config, input_path=fp32_path, output_path=fp32_opt_path, world_size=world_size)

            # Re-assign default FP model path as the optimized version
            fp_path = fp32_opt_path

            logger.info(
                f"The {args.model_name} ONNX model has been successfully optimized with the ORT transformer optimizer script!"
            )

            # Change precision of exported models from FP32 to FP16
            if args.precision == Precision.FLOAT16 or (args.precision == Precision.INT4 and args.execution_provider != "cpu"):
                fp_path = convert_to_float16(args, fp_path, rank)

            if args.precision == Precision.INT4:
    ...
    args.precision = (
        "fp32"
        if args.precision == Precision.FLOAT32 or (args.precision == Precision.INT4 and args.execution_provider == "cpu")
        else "fp16"
    )
